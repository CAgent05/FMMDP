...
def proto_neg_train_model(trainloader, train_label, test_data, test_label, input_size,args):
  print("input size: ", input_size, "train label shape: ", len(np.unique(train_label)))
  ...
  save_dir = './model/COSCO/'
  if not os.path.exists(save_dir):
      os.makedirs(save_dir)
  centroids_file = os.path.join(save_dir, args.dataset + '_train_centroids.pt')
  torch.save(train_centroids, centroids_file)

  weights_file = './model/COSCO/{}.pth'.format(args.dataset)
  torch.save(model_resnet.state_dict(), weights_file)

  #test_data = test_data.cpu().numpy()
  #test_data = np.load(data_dir + dataset_name + 'X_test.npy')
  test_data = torch.from_numpy(test_data).float()
  test_data = test_data.cuda()
  print("Test data shape: ", test_data.transpose(1,2).shape)
  ...
  train_centroids = torch.load(centroids_file)
