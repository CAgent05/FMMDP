...
try:
    ...
    # 如果代码中使用了topk函数，需要自定义实现
    def topk(x, ratio, batch, min_score=None, tol=1e-7):
        # 这里需要根据原始topk函数的用法来实现
        # 或者使用TopKPooling类的相关方法
        pass
    ...
    def __init__(self, Heads, Dim_input, N_output, Dim_output, use_cuda):
        ...
        if self.use_cuda == 1:
            self.input2centroids_weight = torch.nn.Parameter(
                torch.zeros(hiden_channels, 1).float().to(A_device), requires_grad=True)
            self.input2centroids_bias = torch.nn.Parameter(
                torch.zeros(hiden_channels).float().to(A_device), requires_grad=True)
        else:
            self.input2centroids_weight = torch.nn.Parameter(
