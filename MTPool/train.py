...
parser.add_argument('--dataset', type=str, default="BipedalWalkerHCSA_20",
...
def train():
    ...
    for epoch in range(args.epochs):
        ...
        if torch.isnan(output).any() or torch.isnan(loss_train):
            print(">>> nan detected in output or loss!")
            breakpoint()   # 或者直接 raise
    ...
    model_dir = "/home/jc/CY/FMMDP/model/MTPool/"
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
    model_path = os.path.join(model_dir, args.dataset + ".pt")
