...
# GPU monitoring library imports
try:
    import pynvml
    pynvml.nvmlInit()
    NVML_AVAILABLE = True
...
class GPUMonitor:
    """Enhanced GPU monitoring class supporting multiple monitoring methods"""
    ...
    def get_gpu_metrics(self):
        """Get GPU metrics, prioritizing NVML, falling back to nvidia-smi"""
        # Try NVML first
        ...
        # Both failed, return default values
        return {
            'gpu_utilization': 0,
...
class ResourceMonitor:
    ...
    def get_gpu_metrics(self):
        ...
        # Get current process GPU memory usage (via nvidia-smi)
    ...
    def collect_metrics(self, step=None, epoch=None, phase=None, loss=None, acc=None):
        ...
        # Update carbon emission tracking
        self.carbon_tracker.update_energy(gpu_metrics['power_usage_w'], 1)  # Assume 1 second interval
        
        # Collect CPU/memory metrics
        cpu_memory_metrics = self.get_cpu_memory_metrics()
        metrics.update(cpu_memory_metrics)
        
        # Collect PyTorch metrics
    ...
    def save_human_readable_summary(self, filepath):
        ...
        try:
            with open(filepath, 'w') as f:
                ...
                # Display different content based on GPU monitoring success
    ...
    def get_summary_stats(self):
        ...
        # Only add GPU metrics when GPU monitoring succeeds
        ...
        # Only add power metrics when power monitoring succeeds
...
class CarbonTracker:
    def __init__(self, region='CN'):
        # Carbon intensity coefficients (g CO2/kWh)
    ...
    def get_carbon_emission(self):
        """Calculate total carbon emission (kg CO2)"""
    ...
    def get_equivalent_metrics(self):
        """Convert to intuitive equivalent metrics"""
        carbon_kg = self.get_carbon_emission()
        # Equivalent comparisons
        car_miles = carbon_kg / 0.404  # Each mile of car travel emits about 0.404kg CO2
        tree_months = carbon_kg / (21.77 / 12)  # One tree absorbs about 1.81kg CO2 per month
...
class ModelComplexityAnalyzer:
    ...
    def estimate_model_size(model):
        ...
        # Assume each parameter is float32 (4 bytes)
...
parser = argparse.ArgumentParser(description='Traing todynet with resource monitoring')
...
parser.add_argument('--gpu', default=0, type=int,
                    help='GPU id to use.')
# New monitoring-related parameters
...
def main_work(args):
    ...
    if not torch.cuda.is_available():
        ...
        # Record metrics after model loading
    ...
    for epoch in range(args.epochs):
        ...
        # remember best acc - safely handle tensor
    ...
    # log & print the best_acc - safely handle tensor
...
def train(train_loader, model, criterion, optimizer, lr_scheduler, args, monitor, epoch):
    ...
    for count, (data, label) in enumerate(train_loader):
        # Record resource status for key batches
        if count % 50 == 0:  # Record every 50 batches
